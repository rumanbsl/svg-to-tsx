...
function cleanupSvg(svg, keepFillColor, extractedWidth = "", extractedHeight = "") {
  ...
  const cleanedSvg = hasViewBox ? svgLink.replace(/viewBox/, `${replacedString} viewBox`) : svgLink.replace("<svg", `<svg ${replacedString} viewBox="0 0 ${extractedHeight} ${extractedWidth}"`);
