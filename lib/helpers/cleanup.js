...
function _camelCase(string) {
  return string.replace(/^.|-./g, (letter, index) => index === 0 ? letter.toLowerCase() : letter.substr(1).toUpperCase());
}

function _basicCleanup(svg) {
  return svg.replace(/width="\S+"/, '').replace(/height="\S+"/, '').replace(/xmlns="(\S*)"/, '').replace(/data-name="(.*?)"/, '').replace(/([\w-]+)="/g, match => _camelCase(match)).replace(/\s{2,}/g, ' ').replace(/xlink\:href="(\S*)"/g, 'xlinkHref="$1"').replace(/xmlns\:xlink="(\S*)"/g, 'xmlnsXlink="$1"').replace(/<style>(.*?)<\/style>/g, '');
}
...
function cleanupSvg(svg, keepFillColor, extractedWidth = "", extractedHeight = "") {
  ...
  const svgLink = _basicCleanup(svg);
  ...
  const cleanedSvg = hasViewBox ? svgLink.replace(/viewBox/, `${replacedString} viewBox`) : svgLink.replace("<svg", `<svg ${replacedString} viewbox="0 0 0 ${extractedHeight} ${extractedWidth}"`);
...
function cleanupNativeSvg(svg, keepFillColor) {
  const cleanedSvg = _basicCleanup(svg).replace(/viewBox/, '{...rest} height={height || size} width={width || size} style={style} viewBox').replace(/\<[a-z]|\<\/[a-z]/g, match => match.toUpperCase());
